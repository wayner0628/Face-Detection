...
class Adaboost:
    ...
    def selectBest(self, featureVals, iis, labels, features, weights):
        ...
        """--------------Explanation---------------
        The "clfs" list would contain all of the weak classifier built for every 
        features sent into the selectBest funtion.

        Then for every weak classifier, creating a place in list "error" to store 
        every classifier's epson value sum toward a given set of data.
        When checking a classifier's classification result to an image(integral 
        image), I wrote a function in classifier.py called "decision()" for fast
        calculation. By sending the feature value to classifier and let it justify
        whether it is a face, we can avoid compute feature values twice, which 
        really cost a great deal of time :(. 

        After the steps above, now we have every classifier's epson value, things
        to do is picking up the classifier with the lowest error, and return this 
        classifier class and its epson value back.

        #Method 2: Everytime the classifier has done a wrong decision, its mul 
        value would plus one, and when we are picking the best classifier, the 
        classfier with the lowest epson value multiply mul value would be selected.
        The goal is to avoid picking some classifier that can have a low epson 
        value under some extreme circumstances, its overall correctness will also
        be taken into our consideraion.
        """
        error = []
        clfs = []
        #mul = []
        for i in range(len(features)):
          clfs.append(WeakClassifier(features[i],0,1))
        for i in range(len(features)):
          error.append(0.0)
          #mul.append(0)
          for j in range(len(weights)):
            if(clfs[i].decision(featureVals[i,j])!=labels[j]): 
              error[i] += weights[j]
              #mul[i] += 1
          
        best = 0
        bestError = error[0] #* mul[0]
        for i in range(len(error)):
          #if(error[i] * mul[i] < bestError):
          if(error[i] < bestError):
            bestError = error[i] #* mul[i]
            best = i
        bestClf = clfs[best]
